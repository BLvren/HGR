...
class aceModel_g(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(n_CLASS, 512)
...
def test_Hscore(test_loader, model):
    ...
    write_dir = './withoutbg/resnet18/test/'

    testaccfile = write_dir + 'testacc'
    ...
    #data_write_csv('testlossfile' + '.csv', lossData)
    data_write_csv(testaccfile + '.csv', accData)
...
def train_Hscore(train_loader, model, optimizer, epoch, file_dir_path):
    ...
    write_dir='./withoutbg/resnet18/train/'

    trainlossfile= write_dir+'trainloss'

    trainaccfile = write_dir+ 'trainacc'
    ...
    data_write_csv(trainlossfile+'.csv',lossData)
    data_write_csv(trainaccfile+'.csv',accData)
...
def train_model_with_Hscore(train_loader,test_loader,model,args,file_dir_path):
    ...
    for epoch in range(args.nb_epochs):
        ...
        #data_write_csv(trainlossfile+'_train_loss_all' + '.csv', loss_list)
        #data_write_csv(trainaccfile + '_train_acc_all' +'.csv', acc_list)

        end_time = time.clock()
        print("Training Time:{:.2f}".format(end_time - start_time))
        acc=test_Hscore(test_loader, [model_f,model_g])
        #loss_list.append([loss.cpu() .detach().numpy()])
        acc_list.append([acc])
        #data_write_csv(trainlossfile +'_test_loss_all' + '.csv', loss_list)
        #data_write_csv(trainaccfile + '_test_acc_all' +'.csv', acc_list)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d","--dataset",type=str, default='image', choices=['mnist','cifar10','image'], help="the dataset")
